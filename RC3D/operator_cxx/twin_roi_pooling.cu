...
__global__ void TWinROIPoolForwardKernel(const int count, const Dtype* bottom_data,
                                     const float temporal_scale_, const int channels,
                                     const int twin_length,
                                     const int height, const int width,
                                     const int pooled_height, const int pooled_width,
                                     const Dtype* bottom_rois, Dtype* top_data,
                                     Dtype* argmax_data) {
  const int pooled_length = 1;
...
//    int roi_end_h = round(bottom_rois[4] * spatial_scale);
    ...
    int roi_length = max(roi_end_time - roi_start_time + 1, 1);
...
template<typename Dtype>
inline void TWinROIPoolForward(const Tensor<gpu, 4, Dtype> &out,
  ...
  MSHADOW_CUDA_POST_KERNEL_CHECK(TWinROIPoolForwardKernel);
}

template<typename Dtype>
__global__ void TWinROIPoolBackwardAccKernel(const int count, const Dtype* top_diff,
...
template<typename Dtype>
inline void TWinROIPoolBackwardAcc(const Tensor<gpu, 4, Dtype> &in_grad,
...
inline void TWinROIPoolForward(const Tensor<gpu, 4, Dtype> &out,
                           ...
                           const Tensor<gpu, 4, Dtype> &max_idx,
                           const float spatial_scale) {
  cuda::TWinROIPoolForward(out, data, bbox, max_idx, temporal_scale_);
}
...
inline void TWinROIPoolBackwardAcc(const Tensor<gpu, 4, Dtype> &in_grad,
                               ...
                               const Tensor<gpu, 4, Dtype> &max_idx,
                               const float spatial_scale) {
  cuda::TWinROIPoolBackwardAcc(in_grad, out_grad, bbox, max_idx, temporal_scale_);
}
...
template<>
Operator* CreateOp<gpu>(TWinROIPoolingParam param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new TWinROIPoolingOp<gpu, DType>(param);
  });
  return op;
}
